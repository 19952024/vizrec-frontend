...
export default function OAuthButtons({ variant = "signin" }: Props) {
  ...
  return (
    <>
      {message && (
        <div className="mb-6 rounded-xs border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800 dark:border-amber-800 dark:bg-amber-900/20 dark:text-amber-300">
          {message}
        </div>
      )}
      <button
      ...
      >
        <span className="mr-3">
          <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <defs><clipPath id="clip0_95:967"><rect width="20" height="20" fill="white" /></clipPath></defs>
          </svg>
        </span>
        Sign in with Google
      </button>
